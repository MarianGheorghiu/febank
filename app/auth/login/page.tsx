...
import { useUser } from "@/app/context/UserContext"; // <- Importăm hook-ul global de autentificare

export default function LoginPage() {
    const [email, setEmail] = useState(""); // <- State adăugat pentru Email
    const [password, setPassword] = useState(""); // <- State adăugat pentru Parolă
    const [showPassword, setShowPassword] = useState(false);
    const [isLoading, setIsLoading] = useState(false);

    const router = useRouter();
    const { login } = useUser(); // <- Extragem funcția de login din context

    const handleLogin = (e: React.FormEvent) => {
        e.preventDefault();
        if (isLoading) return;
        if (!email || !password) return; // Validare de bază direct la poartă
        ...
        setTimeout(() => {
            ...
            // Extragem dinamic un nume curat din email pentru personalizarea interfeței
            // Exemplu: aurel@mbank.com -> Aurel | mrn@admin.com -> Mrn
            const prefix = email.split("@")[0];
            const derivedName =
                prefix.charAt(0).toUpperCase() + prefix.slice(1);

            // Inițializăm sesiunea globală. Funcția calculează automat dacă e admin sau client,
            // salvează sesiunea securizată și face router.push("/dashboard")
                ...
                <GlassCard className="relative !p-5 sm:!p-10 space-y-8 shadow-[0_40px_90px_rgba(0,0,0,0.8)] overflow-hidden">
                    ...
                    <form className="space-y-6" onSubmit={handleLogin}>
                        <Input
                            ...
                            icon={<Mail size={16} />}
                            disabled={isLoading}
                            value={email} // <- Controlat de React State
                            onChange={(e) => setEmail(e.target.value)} // <- Salvează valoarea introdusă
                            required
                        />
                        ...
                        <div className="relative w-full">
                            <Input
                                ...
                                className="pr-12"
                                disabled={isLoading}
                                value={password} // <- Controlat de React State
                                onChange={(e) => setPassword(e.target.value)} // <- Salvează valoarea introdusă
                                required
                            />
                            <button
                                type="button"
